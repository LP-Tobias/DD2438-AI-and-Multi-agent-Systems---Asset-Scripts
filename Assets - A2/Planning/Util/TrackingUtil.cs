...
namespace MAS2024.Assignment2
{
    class TrackingUtil
    {
        ...
        public static void concatenatePositionsAndTimes(ref List<Vector2> positions1, ref List<float> times1, List<Vector2> positions2, List<float> times2)
        {
            for (int i = 0; i < times2.Count; i++)
            {
                times2[i] += times1[times1.Count - 1];
            }   
            positions1.AddRange(positions2);
