...
namespace MAS2024.Assignment2
{
    class RRT
    {
        ...
        public float goalDistance = 2.5f; // Distance from goal to be considered reached
        ...
        public (List<Vector2>, List<float>) FindPath(Vector3 startPos3D, Vector3 goalPos3D, Vector3 startVel3D, CollisionDetector detector, float timeLimit=20f)
        {
            ...
            Vector2 startPos = new Vector2(startPos3D.x, startPos3D.z);
            Vector2 goalPos = new Vector2(goalPos3D.x, goalPos3D.z);
            Vector2 startVel = new Vector2(startVel3D.x, startVel3D.z).normalized * 2f;
            ...
            while (true)
            {
                ...
                // if (Vector2.Distance(p, goalPos) < goalDistance)
                // {
                //     p = goalPos;
                // }
            ...
            (positions, times) = endNode.pathFromParent.Last().Render(0.5f, localPlanner.acceleration, localPlanner.deceleration, 0.1f);
