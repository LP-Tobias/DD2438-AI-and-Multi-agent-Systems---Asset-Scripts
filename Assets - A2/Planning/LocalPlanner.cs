...
namespace MAS2024.Assignment2
{
    class LocalPlanner
    {
        ...
        public float maxTurnRadius = 1000000f; // Maximum turn radius allowed before being considered a straight line
        ...
        public float minPathLength = 5f; // Shortest path length to generate

        float epsPathLength = 0.1f; // Shortest path segment length where numerical errors start to happen
        ...
        public List<PathSegment> FindPathRelaxed(Vector2 p1, Vector2 p2, Vector2 v0, PathSegment parent=null)
        {
            ...
            // Cannot drive to point, use circle path
            if (D < radius)
            {
                // Calculate the circle segment that intersects the point
                // This version lets us always drive exactly to the sampled point
                float d = p.magnitude;
                float newRadius = 0.5f * d / Mathf.Sin(rawAngle);
                float newAngle = 2 * rawAngle;

                float length = newAngle * newRadius;

                if (newAngle > 0.5f * Mathf.PI || newRadius < minTurnRadius || newRadius > maxTurnRadius || length < minPathLength)
                {
                    return null;
                }
                
                float maxSpeed = MaxSpeedOnCurve(newRadius);

                var path = new CirclePathSegment(maxSpeed, length, acceleration, deceleration, parent);
                path.radius = newRadius;
                path.sgn = sgn;
                path.angle = newAngle;
                path.c = p1 - sgn * newRadius * Rot90(vNorm);
                path.p1 = p1;
                path.p2 = p2;
                path.endVel = Rot90(-sgn * (path.p2 - path.c));
            ...
            else
            {
                ...
                if (angle > 0.5f*Mathf.PI || lineLength < epsPathLength)
                {
                    return null;
                }
                ...
                if (Vector2.Angle(circlePath.endVel, linePath.endVel) > 10)
                {
                    Debug.Log($"Something strange: {circlePath.radius}, {linePath.length}");
                }
        ...
        public List<PathSegment> FindDubinsPath(Vector2 p1, Vector2 p2, Vector2 v1, Vector2 v2, PathSegment parent = null)
        {
            ...
            if (angle1 > 0.5f * Mathf.PI || angle2 > 0.5f * Mathf.PI || lineLength < epsPathLength)
            {
                return null;
            }
